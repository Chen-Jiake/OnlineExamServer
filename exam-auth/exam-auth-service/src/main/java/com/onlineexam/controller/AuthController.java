...
public class AuthController {
    ...
    public ResultBean<UserInfo> loginQ(
        ...
        CookieUtils.setCookie(request, response, "Q_TOKEN", token, jwtProperties.getExpire() * 60);
        UserInfo userInfo = null;
        try {
            userInfo = JwtUtils.getInfoFromToken(token, jwtProperties.getPublicKey());
        } catch (Exception e) {
    ...
    public ResultBean<UserInfo> verifyQ(
            HttpServletRequest request,
            HttpServletResponse response,
            @CookieValue("Q_TOKEN") String token) {
        try {
            UserInfo userInfo = JwtUtils.getInfoFromToken(token, jwtProperties.getPublicKey());
    ...
    public ResultBean<UserInfo> logoutQ(
        ...
        try {
            CookieUtils.setCookie(request, response, "Q_TOKEN", "", 0);
